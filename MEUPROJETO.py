...
def inserir_usuario():
    ...
    if nome and email:
        conn = conectar()
        c = conn.cursor()
        c.execute('INSERT INTO usuarios(id,nome, email, telefone, endereco) VALUES(?,?,?)', (nome, email, telefone, endereco))
        conn.commit()
        conn.close()
...
btn_consulta = tk.Button(janela, text='consulta', command=editar)
btn_consulta.grid(row=7, column=0, padx=10, pady=10)
